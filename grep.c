...
int main(int argc, char *argv[]) {
	...
	char nl[2] = "\n"; /*Terminating character for tokenisation */
   	char *token;
	if(argc == 1) {
		printf("Use ./Grep -h command for usage\n");
		exit(1);
	}
	...
	if(strcmp(argv[1],"-r") == 0 || strcmp(argv[1], "-ri") == 0 || strcmp(argv[1], "-rv") == 0) {
		...
		/*To handle cases without entering directory name*/
		else{
			for(i = 1; i <= 10; i++) {
	...
	/*5) -c command + extra options  : -cv or -vc*/
	 	...
	 	|| strcmp(argv[1], "-iL") == 0 || strcmp(argv[1], "-Li") == 0)
		{
		 ...
		 for(i = 1; i<= 10; i++){
            ...
            if(strcmp(argv[1], "-Li") == 0 || strcmp(argv[1], "-iL") == 0 || strcmp(argv[1], "-li") == 0 || strcmp(argv[1], "-il") == 0){
                    while(token != NULL){
                            /* Assigning FLAG values to all lines*/
                            if(mystrstrcase(token, argv[2]) != NULL){
                                FLAG = 1;
                                break;
                            }
                    ...
                    /* For combination of  -l & -i command for loop*/
                    if(FLAG == 1 && (strcmp(argv[1], "-li") == 0 || strcmp(argv[1], "-il") == 0)){
                            printf("%s%s\n", CYN, argv[i+2]);
                    }
                    /* For loop for combination -L & -i command*/
